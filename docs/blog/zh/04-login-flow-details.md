...
回头看，四条链路都遵守同一套约束：**不加开关、除 `contextMenus` 外不加权限、不改存储结构与加密格式、不动既有条目的语义**；新增的用户可见文案全部中英双语成对提交；每个功能都带回归测试，全仓库 1337 项自动化测试（分布在 119 个测试文件）与 `pnpm build`（Chromium）+ `pnpm build:firefox`（Firefox）双端构建作为验收线。
