...
2. **D1（周二~周四）**：上午掘金 + 知乎发布中文三篇（间隔 1-2 小时避免被判批量）。
3. **D1 晚**：Dev.to 发布英文三篇。
4. **D2**：Show HN + r/SideProject；GitHub README 加「Blog」入口（见首页页脚链接已就绪）。
5. **D3+**：微信公众号同步中文三篇；把各平台链接回填到下方表格。
