...
- 保活门控按平台差异化：Windows 常驻保活（含会话失效期，避免冷启动数秒白屏）；非 Windows 会话有效期内保活，失效/清除后进入 30 分钟宽限期保活（期间继续心跳 + 预热 tick，用户活动信号滑动续期），宽限期结束自动收敛停活，不增加常态电量开销。详见下方「侧边栏秒开跨平台策略」。
...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| SW 保活（`syncSwKeepaliveAlarm`）             | 常驻保活：20s 心跳 + 0.5min 复活闹钟，**会话失效后也不停**                                       | 会话有效期内保活；失效/清除后进入 30 分钟宽限期保活（窗口聚焦 / Tab 激活 / 预唤醒消息滑动续期，用户持续活跃期间保活随活跃延续；仅当用户离开且 30 分钟内无任何活动信号时才自动收敛停活） | 浏览器首启 10 分钟引导期窗口强制保活；平台判定不可得（null）时 fail-safe 维持现状 |
| 渲染资源预热（`maybeWarmSidePanelResources`） | 全量四层预热（HTML → 静态资源 → 动态 chunk → 二级依赖，约 25 文件）/ 5min 节流                   | 轻量预热（HTML + module/modulepreload/CSS + 白名单认证视图关键 chunk 及其二级依赖，约 12 文件）/ 5min 节流                                                                              | 浏览器首启与扩展安装/更新时经 `ignorePlatformGate` 跨平台全量预热                 |
...
- Mac SSD 快、无杀软扫描放大，痛点在于「会话失效后 SW 死亡 → 预热停止 → 文件被 OS 磁盘缓存逐出（长时间闲置/系统休眠后尤甚）→ 下次打开撞三冷叠加白屏」（会话失效宽限期保活覆盖，活动信号滑动续期适配任意间隔）、「重启后三冷叠加」（引导期窗口覆盖），常驻保活只会徒增常态电量开销；轻量预热额外按白名单保留认证视图 chunk（含 Element Plus CSS 运行时，认证态最大冷读单体），根治 macOS 磁盘缓存逐出后认证态打开的数秒白屏；
