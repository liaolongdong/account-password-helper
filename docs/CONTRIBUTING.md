...
> ⚠️ 跑单个测试文件要用 `pnpm exec vitest run <file>`。`pnpm test:run -- <file>` 里的 `-- <file>` **不会**被转发给 vitest CLI（实测仍跑全部 101 个文件），因为它不是 `vitest run <file>` 那种位置参数形式。
