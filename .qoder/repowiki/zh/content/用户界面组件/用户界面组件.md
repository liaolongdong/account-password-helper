...
- [components/BrandLogo.vue](file://components/BrandLogo.vue)
...
## 更新摘要
**变更内容**
- 新增BrandLogo品牌标识组件，提供统一的品牌视觉标识
- 新增ValiditySettingDialog有效期设置对话框组件，支持会话有效期管理
- 新增ValidityHoursSelect有效期选择器组件，提供标准化的有效期选项
- 扩展Vue 3响应式系统在侧边栏组件中的应用，实现实时状态管理
...
**更新** 新增了完善的Vue 3响应式系统，包括响应式变量管理、计算属性、监听器等，显著提升了组件的实时更新能力和用户体验。同时新增了统一的对话框样式系统，包括全屏宽度对话框和固定宽度对话框的设计规范，以及Shadow DOM样式处理工具。新增的品牌标识系统和有效期管理系统进一步完善了用户体验和功能完整性。

## 项目结构
- 入口层
  - options：选项页，承载主密码设置/验证、密码列表管理、导入导出、有效期设置等功能
  - popup：弹出页，提供快速入口与基础操作
  - sidepanel：侧边栏，面向页面内自动填充场景，使用Vue 3响应式系统
  - background：后台脚本，负责快捷键、侧边栏开关、消息路由
  - content：内容脚本，负责表单检测与自动填充触发
- 组件层
  - 品牌标识：BrandLogo，统一的品牌视觉标识
  - 可复用对话框：主密码设置/验证、密码表单、导入 Excel、有效期设置
  - 专用选择器：ValidityHoursSelect，标准化的有效期选项
  - Vue 3响应式系统：useChromeListeners、useSessionTimer composable，管理监听器生命周期和会话定时器
...
ValidityDlg["有效期设置对话框(ValiditySettingDialog.vue)"]
ValiditySelect["有效期选择器(ValidityHoursSelect.vue)"]
ChromeListeners["Chrome监听器(useChromeListeners.ts)"]
SessionTimer["会话定时器(useSessionTimer.ts)"]
end
subgraph "样式系统"
...
Background --> Types
Content --> Types
ValidityDlg --> ValiditySelect
ValidityDlg --> SessionTimer
ImportDlg --> FullWidthCSS
MasterDlg --> FullWidthCSS
VerifyDlg --> FullWidthCSS
FormDlg --> FixedWidthCSS
FormDlg --> ShadowDom
ValidityDlg --> FullWidthCSS
```

**图表来源**
- [entrypoints/options/App.vue:1-793](file://entrypoints/options/App.vue#L1-L793)
- [entrypoints/popup/App.vue:1-233](file://entrypoints/popup/App.vue#L1-L233)
- [entrypoints/sidepanel/App.vue:1-1002](file://entrypoints/sidepanel/App.vue#L1-L1002)
...
- [components/BrandLogo.vue:1-64](file://components/BrandLogo.vue#L1-L64)
- [components/ValiditySettingDialog.vue:1-135](file://components/ValiditySettingDialog.vue#L1-L135)
- [components/ValidityHoursSelect.vue:1-50](file://components/ValidityHoursSelect.vue#L1-L50)
...
- 选项页（App.vue）
  ...
  - **新增** 集成BrandLogo品牌标识和ValiditySettingDialog有效期设置对话框
- 弹出页（App.vue）
  - 快速入口：打开选项页、打开侧边栏
  - 会话有效时显示密码数量
  - **新增** 集成BrandLogo品牌标识
...
- 对话框组件
  ...
  - **新增** 有效期设置：全屏对话框，管理会话有效期和清除会话
  - 免责声明：提示性信息展示
- **新增** 品牌标识组件
  - BrandLogo：纯SVG钥匙形状品牌标识，支持尺寸自定义
  - 统一品牌视觉，跟随父级颜色
- **新增** 有效期管理组件
  - ValiditySettingDialog：有效期设置对话框，展示会话状态并允许修改
  - ValidityHoursSelect：标准化有效期选择器，提供1/2/4/8/12/24小时选项
...
- 组件通信
  ...
  - **新增** 会话定时器通信：useSessionTimer管理有效期状态和定时更新
...
**图表来源**
- [entrypoints/popup/App.vue:104-149](file://entrypoints/popup/App.vue#L104-L149)
- [entrypoints/background.ts:34-138](file://entrypoints/background.ts#L34-L138)
- [entrypoints/sidepanel/App.vue:101-102](file://entrypoints/sidepanel/App.vue#L101-L102)
...
- 对话框集成
  ...
  - **新增** 有效期设置：全屏弹窗，管理会话有效期和清除会话
- 状态与事件
  - 计算属性：搜索关键词、过滤后的密码列表
  - 事件：导入完成、保存完成、批量删除、排序变更
  - **新增** 会话定时器：useSessionTimer管理有效期状态和定时更新
...
SaveForm --> Refresh
Main --> Validity["点击有效期设置"]
Validity --> ValidityDlg["有效期设置对话框"]
ValidityDlg --> SelectHours["选择有效期"]
SelectHours --> SaveValidity["保存设置"]
SaveValidity --> Refresh
Setup --> Verify
```

**图表来源**
- [entrypoints/options/App.vue:646-657](file://entrypoints/options/App.vue#L646-L657)
- [components/ValiditySettingDialog.vue:1-135](file://components/ValiditySettingDialog.vue#L1-L135)
- [components/ValidityHoursSelect.vue:1-50](file://components/ValidityHoursSelect.vue#L1-L50)
- [components/ImportDialog.vue:1-343](file://components/ImportDialog.vue#L1-L343)
- [components/PasswordFormDialog.vue:1-336](file://components/PasswordFormDialog.vue#L1-L336)
...
- 认证与数据
  ...
  - **新增** BrandLogo品牌标识：统一品牌视觉，提升用户体验
- 填充流程
...
- **新增** 有效期设置对话框
  ...
  - 集成ValidityHoursSelect选择器，提供标准化选项
  - 支持会话信息实时更新和倒计时显示
- 免责声明
  - 提示性信息展示
...
- **新增** SessionInfo：会话信息接口，包含过期时间和剩余时间

**章节来源**
- [utils/types.ts:1-96](file://utils/types.ts#L1-L96)
- [components/ValiditySettingDialog.vue:101-107](file://components/ValiditySettingDialog.vue#L101-L107)
...
**图表来源**
- [entrypoints/sidepanel/App.vue:192-200](file://entrypoints/sidepanel/App.vue#L192-L200)
...
### 全屏宽度对话框样式系统
**新增** 项目引入了统一的全屏宽度对话框样式系统，专门用于MasterPasswordDialog、PasswordVerifyDialog、ImportDialog、ValiditySettingDialog等组件。
...
- [components/ValiditySettingDialog.vue:1-88](file://components/ValiditySettingDialog.vue#L1-L88)
...
### BrandLogo组件
**新增** BrandLogo是项目的统一品牌标识组件，采用纯SVG实现的钥匙形状设计。

- 设计特点
  - SVG矢量图形：无损缩放，支持任意尺寸
  - 钥匙主视觉：圆环形钥匙头和矩形钥匙杆
  - 透明钥匙孔：通过evenodd路径创建内部透明效果
  - currentColor跟随：颜色通过父级CSS color属性控制
  - 垂直对齐：inline-block和vertical-align middle居中对齐

- 技术实现
  - props配置：支持size参数（默认1em）
  - 默认值：size默认为'1em'，跟随父级字体大小
  - ARIA支持：aria-hidden="true"，不影响屏幕阅读器
  - 响应式：通过width/height属性支持动态尺寸
...
- 样式集成
  - 统一颜色：通过CSS类名控制颜色
  - 尺寸控制：通过CSS类名或内联样式控制大小
  - 品牌一致性：确保所有入口的视觉统一

```mermaid
graph LR
BrandLogo["BrandLogo组件"] --> SVG["SVG图形"]
SVG --> KeyHead["钥匙头圆环"]
SVG --> KeyShaft["钥匙杆矩形"]
SVG --> Teeth["齿状结构"]
KeyHead --> Path1["evenodd路径"]
KeyShaft --> Rect1["矩形"]
Teeth --> Rect2["短齿"]
Teeth --> Rect3["长齿"]
```
...
**章节来源**
- [components/BrandLogo.vue:1-64](file://components/BrandLogo.vue#L1-L64)
- [entrypoints/options/App.vue:678](file://entrypoints/options/App.vue#L678)
- [entrypoints/popup/App.vue:54](file://entrypoints/popup/App.vue#L54)
- [entrypoints/sidepanel/App.vue:9](file://entrypoints/sidepanel/App.vue#L9)

## 有效期管理系统

### ValiditySettingDialog组件
**新增** 有效期设置对话框组件，提供会话有效期管理和状态展示功能。

- 核心功能
  - 会话状态展示：显示当前会话剩余时间
  - 有效期修改：通过ValidityHoursSelect选择新的有效期
  - 会话清除：一键清除当前会话并返回验证状态
  - 实时更新：会话剩余时间每秒自动更新

- 组件结构
  - 头部Alert：介绍有效期设置的作用
  - 会话状态区域：显示当前会话状态和剩余时间
  - 有效期选择：ValidityHoursSelect组件
  - 底部操作：取消和保存按钮
...
- 数据绑定
  ...
  - rules：表单验证规则
  - sessionInfo：会话状态信息
  - loading/clearSessionLoading：加载状态

- 事件处理
  - save：保存有效期设置
  - clearSession：清除当前会话
  - update:modelValue：对话框状态变更

### ValidityHoursSelect组件
**新增** 有效期选择器组件，提供标准化的有效期选项。

- 选项配置
  - 1小时、2小时、4小时、8小时、12小时、24小时
  - 24小时标注"（推荐）"
  - 支持v-model双向绑定

- 技术实现
  - 基于Element Plus Select组件
  - 通过VALIDITY_OPTIONS常量定义选项
  - 支持原生属性透传（v-bind="$attrs"）

- 使用场景
  - ValiditySettingDialog有效期设置
  - 主密码验证时的有效期选择

### useSessionTimer组合式函数
**新增** 会话定时器组合式函数，管理有效期设置和会话状态。

- 核心状态
  - showValiditySetting：有效期设置对话框显示状态
  - validityForm：有效期表单数据
  - validityRules：表单验证规则
  - sessionInfo：会话信息（过期时间、剩余时间）

- 核心功能
  - openValiditySetting：打开有效期设置对话框并初始化数据
  - handleValiditySave：保存有效期设置并更新会话
  - handleClearSession：清除当前会话并重置状态
  - updateSessionInfo：更新会话信息显示
  - startSessionTimer/stopSessionTimer：会话定时器管理
...
participant ValidityDlg as "有效期设置对话框"
participant SessionTimer as "会话定时器"
participant Storage as "本地存储"
User->>Options : 点击有效期设置
Options->>ValidityDlg : 打开对话框
ValidityDlg->>SessionTimer : 获取会话信息
SessionTimer->>Storage : 读取会话数据
Storage-->>SessionTimer : 返回会话信息
SessionTimer-->>ValidityDlg : 显示剩余时间
User->>ValidityDlg : 选择有效期并保存
ValidityDlg->>Storage : 更新有效期设置
Storage-->>ValidityDlg : 保存成功
ValidityDlg->>SessionTimer : 启动定时器
SessionTimer->>ValidityDlg : 每秒更新剩余时间
```

**图表来源**
- [components/ValiditySettingDialog.vue:1-135](file://components/ValiditySettingDialog.vue#L1-L135)
- [components/ValidityHoursSelect.vue:1-50](file://components/ValidityHoursSelect.vue#L1-L50)
- [composables/useSessionTimer.ts:115-200](file://composables/useSessionTimer.ts#L115-L200)
...
## 依赖关系分析
- 组件依赖
  - 选项页依赖导入/表单/主密码/验证对话框、有效期设置对话框与免责声明
  - 侧边栏依赖存储工具类、内容脚本与useChromeListeners composable
  - 弹出页依赖存储工具类
  - **新增** 所有入口依赖BrandLogo品牌标识组件
  - **新增** 选项页依赖useSessionTimer组合式函数管理有效期
- 外部依赖
  - Element Plus：UI 组件库
...
Popup["弹出页"] --> EPlus
SidePanel["侧边栏"] --> EPlus
Options --> BrandLogo["BrandLogo"]
Popup --> BrandLogo
SidePanel --> BrandLogo
Options --> ValidityDlg["ValiditySettingDialog"]
Options --> SessionTimer["useSessionTimer"]
...
- [entrypoints/options/App.vue:678](file://entrypoints/options/App.vue#L678)
- [entrypoints/sidepanel/App.vue:179](file://entrypoints/sidepanel/App.vue#L179)
- [entrypoints/popup/App.vue:54](file://entrypoints/popup/App.vue#L54)
...
- 选项页
  ...
  - **新增** 会话定时器优化：useSessionTimer使用setInterval每秒更新，建议在组件卸载时自动清理
- 侧边栏
...
- 弹出页
  ...
  - **新增** BrandLogo组件优化：SVG矢量图形，无额外资源加载
- 存储与加密
  - AES 加密/解密在会话有效时进行，避免频繁 IO
  - PBKDF2 迭代次数较高，注意在移动端设备上的性能表现
  - **新增** 会话存储优化：useSessionTimer避免频繁读取会话信息
...
- [composables/useSessionTimer.ts:94-113](file://composables/useSessionTimer.ts#L94-L113)
...
- [composables/useChromeListeners.ts:78-108](file://composables/useChromeListeners.ts#L78-L108)
- [components/BrandLogo.vue:44-55](file://components/BrandLogo.vue#L44-L55)
- [composables/useSessionTimer.ts:115-124](file://composables/useSessionTimer.ts#L115-L124)

## 结论
该组件系统以清晰的入口划分与对话框复用实现了高内聚低耦合的架构。通过本地存储与会话管理保障数据安全与用户体验，借助消息通道与内容脚本实现页面内自动填充。**新增的Vue 3响应式系统**显著提升了组件的实时更新能力和用户体验，包括响应式变量管理、计算属性优化、监听器机制等。**新增的对话框样式系统**提供了统一的视觉设计和交互体验，包括全屏宽度对话框和固定宽度对话框两种设计模式，以及Shadow DOM样式处理工具，显著提升了系统的可维护性和用户体验。**新增的品牌标识系统**确保了跨入口的一致视觉体验，而**新增的有效期管理系统**则提供了完善的会话管理功能。建议在大规模数据场景下引入虚拟滚动、批量事务保存与更细粒度的缓存策略，持续优化性能与稳定性。
...
- 使用示例与集成模式
  ...
  - **新增** 品牌标识集成：在所有入口中使用BrandLogo组件
  - **新增** 有效期管理集成：通过useSessionTimer管理会话状态
- 扩展与定制
  ...
  - **新增** 新增品牌标识：参考BrandLogo的设计模式，确保SVG矢量图形和颜色跟随
  - **新增** 新增有效期组件：参考ValiditySettingDialog和ValidityHoursSelect的设计模式
- 响应式设计与无障碍
  - 对话框采用全屏/固定宽度适配移动端
  - 表单输入框提供占位符与标签，提升可访问性
  - **新增** Vue 3响应式系统：使用语义化HTML和适当的ARIA属性
  - **新增** Shadow DOM样式处理：确保样式在各种环境下都能正确应用
  - **新增** BrandLogo组件：支持aria-hidden属性，不影响屏幕阅读器
- 跨浏览器兼容性
  - 依赖 Chrome sidePanel API，需关注版本差异与回退方案
  - **新增** Vue 3响应式系统：确保在不同浏览器版本下的兼容性
  - **新增** Shadow DOM样式处理：使用标准Web API确保兼容性
  - **新增** SVG图形：确保在所有现代浏览器中的兼容性
  - **新增** 会话定时器：使用标准JavaScript API确保兼容性
...
- [components/BrandLogo.vue:44-55](file://components/BrandLogo.vue#L44-L55)
- [components/ValiditySettingDialog.vue:126-133](file://components/ValiditySettingDialog.vue#L126-L133)
- [components/ValidityHoursSelect.vue:40-48](file://components/ValidityHoursSelect.vue#L40-L48)