...
## 更新摘要
**变更内容**
- 更新了错误处理机制：统一了content script的错误处理模式
- 增加了扩展上下文失效检测：在关键操作前检查chrome.runtime.id有效性
- 改进了URL变化通知的错误处理：增加了针对性的用户反馈
- 增强了用户反馈机制：在注入失败、连接失败、填充失败时提供明确的用户提示
...
NeedShow --> |是| CheckContext["检查扩展上下文有效性"]
CheckContext --> ContextOK{"上下文有效？"}
ContextOK --> |是| RequestShow["runtime.sendMessage 显示侧边栏"]
ContextOK --> |否| WarnUser["显示用户警告"]
NeedShow --> |否| Noop["不处理"]
...
**更新** 统一了错误处理模式，增加了扩展上下文失效检测和针对性的用户反馈

- **扩展上下文检测**
  - 在所有关键操作前检查 `chrome.runtime?.id` 是否有效
  - 当检测到扩展上下文失效时，记录警告并重置内部状态
  - 针对 "Extension context invalidated" 错误提供明确的用户提示

- **统一的错误处理模式**
  - content script 的所有异步操作都使用 try/catch 包装
  - 对扩展上下文失效错误进行特殊处理
  - 在 URL 变化通知中改进错误处理逻辑

- **用户反馈机制**
  - 注入失败时显示 "无法在当前页面中注入脚本" 的明确提示
  - 连接失败时显示 "无法连接到页面脚本" 的用户提示
  - 填充失败时提供针对性的错误信息

```mermaid
flowchart TD
ErrorStart["错误发生"] --> CheckContext["检查扩展上下文"]
CheckContext --> IsInvalid{"上下文失效？"}
IsInvalid --> |是| ShowWarning["显示用户警告"]
IsInvalid --> |否| LogError["记录错误日志"]
ShowWarning --> ResetState["重置内部状态"]
LogError --> HandleError["处理具体错误"]
ResetState --> HandleError
HandleError --> UserFeedback["提供用户反馈"]
UserFeedback --> End["完成"]
```

**图表来源**
- [entrypoints/content.ts](file://entrypoints/content.ts#L993-L1028)
- [entrypoints/content.ts](file://entrypoints/content.ts#L1026-L1054)
- [entrypoints/content.ts](file://entrypoints/content.ts#L1195-L1220)
...
## 故障排查指南
- **扩展上下文失效**
  - 现象：控制台显示 "Extension context invalidated" 错误
  - 处理：content script 会自动检测并重置状态，建议刷新页面
  - 预防：避免长时间不使用插件，定期刷新页面
...
## 结论
本项目的消息通信机制围绕 runtime.sendMessage、tabs.sendMessage、runtime.onMessage 构建，形成了清晰的 Content <-> Background <-> SidePanel 三层协作模式。通过统一的消息类型定义、异步响应与错误处理、性能优化手段（防抖、事件委托、MutationObserver），以及完善的侧边栏控制与页面注入流程，实现了稳定高效的扩展通信。

**更新** 最新的错误处理和上下文检测机制进一步增强了系统的健壮性和用户体验。通过扩展上下文失效检测、统一的错误处理模式和针对性的用户反馈，系统能够更好地应对各种异常情况，为用户提供更清晰的操作指导。
