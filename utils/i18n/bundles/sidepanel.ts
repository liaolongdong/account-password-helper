 ...
 * SidePanel 入口语言包注册模块
 *
 * 仅静态内置侧边栏依赖图实际使用的命名空间（common/message/sidepanel/fill/totp/session），
 * 替代全量语言包（575 key）静态打包：侧边栏首屏关键 JS
 * 减少约 100KB（gzip ~35KB），Windows 冷启动的读取/杀软扫描/V8 编译量同步下降。
 * session 命名空间供 SidepanelHeader 的会话剩余时间倒计时（useSessionCountdown）使用。
 *
...
registerMessages('zh-CN', zhCommon, zhMessage, zhSidepanel, zhFill, zhTotp, zhSession as Messages);
registerMessages('en', enCommon, enMessage, enSidepanel, enFill, enTotp, enSession as Messages);
