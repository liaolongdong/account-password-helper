...
- **One-keystroke login, not just fill**: `Ctrl+Shift+F` (Mac `Cmd+Shift+F`) fills the best-matching account for the site and ticks the consent checkbox; turn on "Auto-submit login" in preferences (or use the "Fill and sign in" icon on a side panel entry) and one press runs the whole chain — fill → tick → click login — while GitHub-style two-step logins are continued by the live TOTP code capsule
- **Quadruple fill strategy**: Inline fill (key icon in the input, the default), side panel one-click fill, right-click fill (right-click an input → "Fill Credentials" to fill username/password/2FA code, or generate & fill a strong password), and the quick-fill shortcut — covering everything from deliberate point-and-choose to a blind keypress; failures are reported through an in-page notice + desktop notification + toolbar badge, and a locked session opens an unlock prompt right on the page
