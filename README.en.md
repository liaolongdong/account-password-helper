...
- **Multiple fill modes**: side panel one-click fill, inline fill (key-icon mini panel inside the field), quick-fill shortcut (`Ctrl+Shift+F`); triple fill strategy compatible with React/Vue and other frameworks; optional auto login trigger
...
4. **Shortcuts**: `Ctrl+Shift+P` (open manager), `Ctrl+Shift+L` (toggle side panel), `Ctrl+Shift+F` (quick fill); all customizable at `chrome://extensions/shortcuts` (`Cmd` on Mac)
