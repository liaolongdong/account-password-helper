...
**Q：同一套账号要覆盖整个站点的子域名，怎么办？**

A：密码管理页头部的「跨子域名匹配」提供三档，默认是「仅精确匹配」。切到「精确 + 通配条目」后，网址写成 `*.qq.com` 的账号会在该站任意子域出现；切到「同主域名」再加一层兜底——当前域名没有账号时依次带出主域 `qq.com` 与兄弟子域的条目，并在列表里标注「跨子域」。放宽只改变**哪些账号被展示与可填充**：自动保存的判重口径不随档位改变，`localhost` / `127.0.0.1` 始终按端口区分，侧边栏与内联下拉共用同一套排序、一键填充取首条。

**Q：登录框识别不到，或者填不进去怎么办？**

A：表单藏在自建组件里时（典型是 Web Components 页面），在密码管理页「站点规则」为该域名手动指定账号与密码输入框的 CSS 选择器即可——域名需与登录页完全一致，不支持通配。每条规则默认开启「影子 DOM 穿透」，可关闭以只在主文档内匹配；需要说明的是，任何扩展都读不到 `mode: 'closed'` 的封闭 Shadow DOM，只能进入开放的 Shadow DOM。配好的规则可「导出 JSON」备份或分享给团队（明文，只含域名与选择器，不含账号密码），换机时用「导入 JSON」按域名合并，报告会写明新增、更新与忽略的条数。
...
A：有。密码列表的条目总量上限为 2000 条，按列表条目数计数，回收站里的条目不占额度。达到上限后新增、导入、网页自动保存与回收站恢复都会明确提示被拒绝，不会静默覆盖或删除已保存的数据；导入超出剩余额度时，预览页会显示「还能导入多少条 / 将被忽略多少条」，由你选择只导入前若干条或取消。需要腾出额度时，在密码列表中删除不再使用的条目即可——条目移进回收站的那一刻就释放额度，无需再清空回收站；也可以先用「导出数据」「导出JSON」「加密备份导出」留存全量数据再清理列表。

**Q：条目接近上限时管理页会卡吗？**

A：不会按条目总数卡。列表按页呈现（默认每页 100 条，可切 50 / 100 / 200，档位记在本机、下次打开管理页仍是这一档），一次只渲染当前这一页，所以搜索、输入过滤、翻页、悬停这类交互的耗时基本不再随条目数增长——本机同法实测 600 条与近上限 2000 条两档的读数落在同一量级。打开管理页还包含一次「读整库并解密」，这一步的开销与条目总数相关、不由分页决定，因此这里的口径是「成本不随行数逐条放大」，不是「恒定耗时」。分页只改变"一次画多少行"：搜索、排序、导出与「总计 / 筛选后」读数作用的仍是**全部**命中条目，跨页勾选也会保留（分页条会写明有几条不在当前页）。需要留意的是表头「全选」按表格惯例只勾当前页，要批量处理大量条目时先切到每页 200 条或逐页勾选。
