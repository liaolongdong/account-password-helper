...
- **会话可控**：有效期 1 小时\~7 天可选；支持闲置自动锁定（5\~60 分钟，系统锁屏/屏保激活时立即锁定）、浏览器重启锁定、Popup 一键锁定，过期后敏感字段自动加密回密文；会话剩余时间在密码管理页头部、侧边栏头部与 Popup 常驻展示（剩余 10 分钟内转警示橙、最后 1 分钟内转危急红，点击徽标/胶囊直达有效期设置续期）
- **安全体检**：一键扫描生成 0\~100 综合评分，检测弱密码、密码复用、常见泄露密码（离线字典）、长期未更新、未开两步验证，支持到期提醒
- **安全细节**：剪贴板自动清除（10\~120 秒可选）、密码强度可视化、TOTP 两步验证码本地生成（RFC 6238，零联网），列表/侧边栏实时活码与倒计时、内联面板点击 2FA 徽标即展活码（复制/一键填入），支持扫描网页二维码 / 上传二维码图片一键添加 TOTP 密钥（本地 jsQR 解码，不联网）；两步登录接力：GitHub 式账密页与验证码页分离的两阶段登录，账密填充成功后跳转纯验证码页自动锚定活码胶囊，一键填入 / 复制（3 分钟有效，会话锁定即清除）
...
- **组织能力**：标签多选（颜色稳定一致）、标签筛选（管理页 + 侧边栏）、收藏置顶（上限可配 + LRU 淘汰）、多字段智能搜索（支持拼音/首字母缩写，命中关键词高亮）与排序、一键去重、批量删除/批量编辑标签（追加/移除）/导出选中条目
- **智能录入**：新增表单 URL 自动带入当前活动标签页域名；侧边栏无匹配结果时可一键「添加本站账号」并自动预填当前站点域名
- **防误操作**：回收站 30 天软删除、密码修改历史（可配置启用/关闭，每条最多保留 1~10 份加密快照，默认 5 份，可恢复）、修改主密码原子换钥不丢数据
...
- **秒开体验**：Service Worker 全平台常驻保活 + 密码内存缓存预热 + 侧边栏渲染资源预热，侧边栏打开约 20-50ms 加载完成，会话失效后同样秒开（保活会在后台约每 30 秒唤醒一次扩展，属消除冷启动白屏的主动设计取舍）
...
A：能。删除的密码会先移入回收站保留 30 天，在「数据管理」→「回收站」中可恢复或彻底删除；改错密码也可通过条目的「密码修改历史」（在「安全设置」→「密码历史设置」中可配置启用/关闭及保留条数，默认 5 条加密快照）一键恢复。
