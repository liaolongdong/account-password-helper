...
- **会话可控**：1/2/4/8/12/24 小时会话有效期；会话失效后敏感字段自动加密回密文。
...
1. 安装后点击扩展图标进入密码管理页面
2. 设置主密码并选择会话有效期（1/2/4/8/12/24 小时，默认 24 小时）
3. 在选项页可配置悬浮按钮显示、位置、透明度、是否自动触发登录

### 密码管理

- **新增 / 编辑 / 删除**：选项页提供完整 CRUD
- **批量导入**：Excel 模板导入，提供「下载模板」
...
- 空字段保持原值；解密失败安全降级返回原始数据
- 内存中的主密码副本使用盐值派生的 32 字符密钥二次加密
