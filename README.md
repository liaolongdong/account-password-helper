...
- 📄 **列表分页**：密码列表按页呈现，默认每页 100 条（可切 50 / 100 / 200，档位记在本机、下次打开管理页仍是这一档，回收站列表同档位），接近 2000 条上限时打开管理页不再需要等整张表渲染完；跨页勾选会保留，批量删除 / 改标签 / 导出选中作用的仍是全部勾选项（分页条会写明有几条不在当前页），表头「全选」按表格惯例只勾当前页；新增、编辑、收藏、创建副本后自动跳到该条所在页并高亮，搜索与排序口径变化则回到第 1 页
