...
一款功能强大的 Chrome 浏览器扩展，提供安全、便捷的账号密码管理与自动填充能力。采用 **PBKDF2 + AES-256-GCM** 加密体系，零网络传输，密码绝不出浏览器，支持登录表单智能识别、多策略自动填充、一键自动登录、自动保存账号密码、批量导入导出、加密备份、密码可见性切换、闲时锁定、TOTP两步验证码、密码强度检测、密码生成器、安全体检、回收站、密码修改历史、修改主密码等功能。
...
- **自动保存**：Chrome 式登录凭证捕获，支持登录表单提交、按钮点击、回车提交三种场景；域名白名单/黑名单精准匹配；基于密码库比对的智能去重（相同账号密码不再重复提示，密码变化时弹出「更新」确认）；「不再提示」一键屏蔽域名；跨页面导航凭证不丢失；保存弹窗中可编辑标签和备注。
...
- **安全体检仪表盘**：一键扫描全部密码，生成综合安全评分（0~100）与等级（优秀/良好/一般/较差）；检测弱密码、密码复用（多账号共用同一密码）、常见泄露密码（离线 top-1000 弱口令字典）、长时间未更新（90/180/365 天分级预警）及未开启两步验证条目；支持为条目设置密码到期提醒（N 天后桌面通知）；全程本地计算，零网络传输；支持点击「去处理」直接跳转编辑。
...
- **随机密码生成**：添加/编辑密码时通过魔棒按钮一键生成随机密码，基于 Web Crypto API 保证密码学安全；支持自定义长度（6~50）和字符集（大写/小写/数字/特殊字符），可排除易混淆字符（1/l/I/0/O），生成后实时显示强度进度条；另提供「助记词组（Passphrase）」模式，基于 EFF Diceware 理念从 2048 词库随机组合 3~8 个单词，支持分隔符、首字母大写、末尾追加数字，生成既安全又易记的密码。
- **回收站（软删除）**：删除的密码先进入回收站保留 30 天，支持随时恢复、彻底删除与一键清空；超期条目由后台闹钟自动清理；回收站条目保持密文存储，安全模型与主列表一致。
- **密码修改历史**：每次修改密码自动快照旧密码（加密存储，每条目保留最近 5 条），编辑弹窗中可查看修改时间并一键复制或恢复旧密码，误改密码也能找回。
- **修改主密码**：支持在不丢数据的前提下更换主密码，全部数据（密码/回收站/历史）用新密钥重新加密后单次原子写入，中途失败自动回退不损坏数据；已打开的页面自动采纳新会话密钥，无需重新登录。
...
- **中英文双语界面**：内置中文 / English 运行时切换，无需重启扩展；语言入口位于「偏好设置」面板（与主题风格同组，密码管理页 / 悬浮按钮 / 侧边栏三入口可达），切换后即时生效并自动同步到所有已打开页面；覆盖范围包括扩展页、网页内注入 UI（保存密码弹窗、内联填充面板、页面通知、悬浮按钮）与桌面通知。
...
3. 在选项页点击「偏好设置」按钮可配置主题换肤（6 款色彩主题）、界面语言（中文 / English）、悬浮按钮显示、快速填充方式、自动触发登录、密码可见性切换、透明度等
...
A：在密码管理页「设置」下拉菜单中点击「修改主密码」，验证当前主密码后设置新主密码即可。修改过程中所有数据（密码列表、回收站、修改历史）会用新密钥重新加密并单次原子写入，中途失败不会损坏数据；已打开的页面会自动采纳新会话密钥，无需重新登录。注意：修改主密码不影响已导出的加密备份文件（.aph），导入时仍用导出时的密码解密。
...
A：安全体检是密码管理页顶部操作栏提供的一键密码健康扫描功能。点击「安全体检」按钮（旁边有信号灯圆点指示当前健康等级），会打开仪表盘弹窗，展示综合安全评分（0~100 分）和五个维度的检测结果：弱密码、密码复用（多账号共用同一密码）、常见泄露密码（离线 top-1000 弱口令字典命中）、长时间未更新（90/180/365 天分级预警）、未开启两步验证。长时间未更新的条目还可设置密码到期提醒（7/30/90 天后桌面通知提醒更换）。每条问题都提供「去处理」按钮，点击可直接跳转到对应条目的编辑流程。全程本地计算，不联网、不上传任何数据。
...
**Q：一键填充快捷键（Ctrl+Shift+F）是怎么工作的？**

A：按下 `Ctrl+Shift+F` / `Cmd+Shift+F` 后，插件会直接填充与侧边栏列表首条一致的账号密码（域名匹配优先 + 收藏置顶 + 排序配置），无需打开侧边栏。多条匹配时通知中会告知填充了哪条、共几条匹配，可打开侧边栏切换其他账号。未验证主密码、无匹配账号、页面未就绪时均会通过桌面通知提示；填充结果同时通过工具栏图标角标反馈（成功绿色对勾 / 失败红色感叹号）。

**Q：为什么快捷键默认没有生效，需要手动设置？**

A：Chrome 出于安全考虑，不会为「通过更新新增的命令」自动绑定建议快捷键（避免更新时被静默劫持）。因此在快捷键功能上线前就已安装本插件的用户，以及开发环境下重新加载的扩展，快捷键会显示为「未设置」，只需按上一条手动绑定一次即可长期生效。Popup 中未绑定的入口会显示「设置快捷键」按钮，点击可直达设置页。
...
A：能。删除的密码会先移入回收站保留 30 天，在密码管理页「数据管理」下拉菜单 →「回收站」中可查看已删除条目，支持单条「恢复」、「彻底删除」和「清空回收站」。超过 30 天的条目会被自动清理；彻底删除会同步清理该条目的密码修改历史。回收站条目保持加密存储，锁定态下仅显示占位符。

**Q：改错密码了能恢复旧密码吗？（密码修改历史）**

A：能。每次修改密码时，插件会自动将旧密码加密快照入历史（每个条目保留最近 5 条）。在编辑该条目的弹窗底部可看到「密码修改历史」区块，每条显示修改时间，点击「复制」可复制旧密码，点击「恢复」即将旧密码回填到密码框。历史以加密态存储，查看需会话有效。
...
A：有三种方式进入偏好设置面板：①在密码管理页点击「偏好设置」按钮；②点击页面悬浮按钮上的齿轮图标；③在侧边栏快速填充右上角齿轮图标。可配置：主题换肤（6 款色彩主题）、界面语言（中文 / English）、悬浮按钮显示/隐藏、快速填充方式、自动触发登录、密码可见性切换以及按钮透明度。

**Q：如何切换界面语言（中文 / English）？**

A：在偏好设置面板的「语言 / Language」分组中点击「中文」或「English」即可切换，切换后即时生效并通过 storage 自动同步到所有已打开的插件页面（管理页 / 侧边栏 / Popup），无需刷新。
...
A：在添加或编辑密码的表单中，密码输入框旁有一个魔棒（MagicStick）图标按钮，点击即可打开密码生成器，顶部可在「随机密码」与「助记词组」两种模式间切换。随机密码模式可自定义密码长度（6~50）、选择字符集（大写/小写/数字/特殊字符），还可排除易混淆字符（1、l、I、0、O）；助记词组模式从内置 2048 词库随机选取 3~8 个单词组合（如 `Apple-River-Cloud-Tiger42`），支持分隔符、首字母大写、末尾追加数字，生成既安全又易记的密码。生成后实时显示密码强度进度条，点击「使用此密码」即可填入。密码生成基于 Web Crypto API（crypto.getRandomValues），保证密码学安全随机性。
