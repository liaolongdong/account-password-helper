...
>
> 📊 **技术亮点**: PBKDF2 600K 迭代 · AES-256-GCM 认证加密 · 侧边栏秒开（缓存快路径 20-50ms）· 6 款主题 · 中英文双语 · 完全离线可用 · 632 项自动化测试
...
- **一键登录，不只是填充**：`Ctrl+Shift+F`（Mac `Cmd+Shift+F`）先把当前站点最匹配的账号填入并自动勾选协议复选框；在偏好设置中开启「自动触发登录」（或使用侧边栏条目的「填充并登录」图标），即可一步走完填充 → 勾选 → 点击登录，GitHub 式两步登录还会自动衔接 TOTP 活码胶囊
- **四重填充策略**：内联填充（输入框钥匙图标，默认）、侧边栏一键填充、右键填充（输入框上右键→「填充账号密码」，可填充用户名/密码/两步验证码，或生成并填充强密码）、快捷键一键填充，覆盖从「精确点选」到「闭眼一键」的不同习惯；失败经页面内提示 + 桌面通知 + 工具栏角标多层反馈，会话失效时就地弹出解锁引导
...
- **高效组织**：标签多选与筛选、收藏置顶（上限可配 + LRU 淘汰）、多字段智能搜索（拼音/首字母缩写，命中高亮）、一键去重、批量删除/编辑标签/导出选中；条目可「查看详情」只读浏览备注全文与密码历史，不必进入编辑态
...
> 📅 文档最后更新：2026-09 · 对应版本 [v3.7.0](https://github.com/liaolongdong/account-password-helper/releases/latest)（最新已发布版本）
