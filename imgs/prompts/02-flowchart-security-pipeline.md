...
5. "Session Lock" — A timer icon with "Auto-lock" label and "5/10/30/60 min" subtitle. When the timer expires, a small "wipe" arrow points to the session key and the decrypted cache being dropped, while the document icons on the disk side stay ciphertext.

CONNECTIONS: Hand-drawn wavy arrows flowing left to right between each step. Each arrow has a small label: "derives" (step 1→2), "produces" (step 2→3), "encrypts" (step 3→4), "protects" (step 4→5). A dashed return arrow from step 5 back to step 4 labeled "on expiry → drop key & cache".

LABELS: "Master Password", "PBKDF2", "600,000 iterations", "256-bit Key", "AES-256-GCM", "Encrypt All", "Session Lock", "Auto-lock", "derives", "produces", "encrypts", "protects", "drop key & cache on expiry"
