    ...
    <div class="help-content">
      <section class="help-section">
        ...
        <ol>
          ...
          <li>
            在密码管理页支持 CSV 导入导出（.csv）、JSON 导入导出（.json），以及多标签（每条最多 3 个，单个最长 30
            字符）、颜色稳定的标签体系。点击星标可收藏常用条目，支持「只看收藏」过滤。
          </li>
          <li>加密备份：在「数据管理」中导出为 .aph 加密文件，导入时解密后可预览数据再确认，安全可靠。</li>
          <li>自动闲置锁定：在「自动锁定设置」中配置闲置时间（5/10/30/60 分钟），超时后自动清除会话并锁定。</li>
          <li>
            收藏上限设置：默认收藏上限 10 条，可在密码管理页「设置」→「收藏上限设置」中调整（1~50
            条）。达到上限后新收藏自动淘汰最近最少使用（LRU）的已收藏条目，确保常用账号始终置顶。
          </li>
          <li>
          ...
          <li>
            随机密码生成：添加/编辑密码时，点击密码框旁的魔棒按钮可一键生成随机密码，支持自定义长度（6~50）和字符集（大写/小写/数字/特殊字符），可排除易混淆字符。
          </li>
          <li>
            剪贴板自动清除：复制密码后可在「剪贴板设置」中配置自动清除延时（10/15/30/60/120
            秒），复制用户名时自动取消密码清除定时器。
          </li>
        </ol>
      </section>
      <section class="help-section">
        ...
        <ul>
          ...
          <li><b>导入 CSV 报错？</b> 请使用「密码管理」页提供的模板下载，确保必填列完整。</li>
          <li>
            <b>悬浮按钮未显示？</b>
          ...
          <li>
            <b>如何备份到邮箱？</b> 密码管理页点击「备份到邮箱」，配置目标邮箱后可一键导出数据
            并唤起邮件客户端，也可开启定时提醒。
          </li>
          <li>
            <b>如何导出/导入加密备份？</b>
          ...
          <li>
            <b>支持从其他密码管理器导入吗？</b>
            支持。在导入弹窗中上传 CSV 文件，插件会自动识别 Chrome、LastPass、Bitwarden、1Password
            的导出格式并映射字段。
          </li>
          <li>
            <b>支持 JSON 格式导入导出吗？</b>
          ...
          <li>
            <b>复制密码后剪贴板会自动清除吗？</b>
            会。在侧边栏复制密码后，插件会根据「剪贴板设置」在指定延时后自动清除。默认 30 秒，可选 10/15/30/60/120
            秒。清除前验证内容未被替换，失焦时采用「尽力清除」策略。复制用户名时自动取消密码清除定时器。
          </li>
          <li>
            <b>如何生成随机密码？</b>
            在添加或编辑密码表单中，点击密码框旁的魔棒按钮打开生成器。可自定义长度（6~50）和字符集（大写/小写/数字/特殊字符），可排除易混淆字符（1/l/I/0/O），生成后实时显示强度进度条，点击「使用此密码」即可填入。
          </li>
          <li>
            <b>如何设置收藏上限？</b>
            密码管理页「设置」→「收藏上限设置」中可调整收藏上限（1~50 条，默认 10 条）。达到上限后新收藏
            将自动淘汰最近最少使用（LRU）的已收藏条目，被淘汰条目仅取消收藏标记、密码数据不受影响。
          </li>
        </ul>
      </section>
